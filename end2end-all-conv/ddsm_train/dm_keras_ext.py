...
def get_dl_model(net, nb_class=3, use_pretrained=True, resume_from=None, 
    ...
    if resume_from is not None:
        print ("Loading existing model state.",sys.stdout.flush())
        model = load_model(resume_from)
        print ("Done.")
    elif net == 'yaroslav':
        model = Yaroslav(classes=nb_class)
    else:
        print ("Loading %s," % (net),sys.stdout.flush())
...
class DMAucModelCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        print (" - Epoch:%d, AUROC:%s, mean=%.4f" % (epoch + 1, str(auc), epoch_auc),sys.stdout.flush())
    ...
    def on_train_end(self, logs={}):
        if self.best_auc >= 0.:
            ...
            print ("\n>>> AUROC was not scored. No model was saved. <<<" ,sys.stdout.flush())
